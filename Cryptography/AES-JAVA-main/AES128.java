public class AES128 {
    private static final int Nb = 4;  // Number of columns in the state
    private static final int Nk = 4;  // Key length (in 32-bit words)
    private static final int Nr = 10; // Number of rounds

    // S-box (substitution box)
    private static final int[] sbox = {
            

    };

    // Round constant (Rcon)
    private static final int[] Rcon = {
            0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36
    };

    private byte[][] state = new byte[4][Nb]; // 4x4 state matrix
    private byte[] key; // 16-byte key
    private byte[][] roundKeys = new byte[Nb * (Nr + 1)][4]; // All round keys

    public AES128(byte[] inputKey) {
        this.key = inputKey;
        keyExpansion();
    }

    // Encrypt a 16-byte block
    public byte[] encryptBlock(byte[] input) {
        loadState(input);
        addRoundKey(0);

        for (int round = 1; round < Nr; round++) {
            subBytes();
            shiftRows();
            mixColumns();
            addRoundKey(round);
        }

        subBytes();
        shiftRows();
        addRoundKey(Nr);

        return dumpState();
    }

    // === Step 1: Key Expansion ===
    private void keyExpansion() {
        for (int i = 0; i < Nk; i++) {
            roundKeys[i][0] = key[4 * i];
            roundKeys[i][1] = key[4 * i + 1];
            roundKeys[i][2] = key[4 * i + 2];
            roundKeys[i][3] = key[4 * i + 3];
        }

        for (int i = Nk; i < Nb * (Nr + 1); i++) {
            byte[] temp = roundKeys[i - 1].clone();
            if (i % Nk == 0) {
                temp = subWord(rotWord(temp));
                temp[0] ^= (byte) Rcon[i / Nk];
            }
            for (int j = 0; j < 4; j++) {
                roundKeys[i][j] = (byte) (roundKeys[i - Nk][j] ^ temp[j]);
            }
        }
    }

    private byte[] rotWord(byte[] word) {
        return new byte[]{ word[1], word[2], word[3], word[0] };
    }

    private byte[] subWord(byte[] word) {
        byte[] result = new byte[4];
        for (int i = 0; i < 4; i++) {
            result[i] = (byte) sbox[word[i] & 0xFF];
        }
        return result;
    }

    // === Step 2: AddRoundKey ===
    private void addRoundKey(int round) {
        for (int c = 0; c < Nb; c++) {
            for (int r = 0; r < 4; r++) {
                state[r][c] ^= roundKeys[round * Nb + c][r];
            }
        }
    }

    // === Step 3: SubBytes ===
    private void subBytes() {
        for (int r = 0; r < 4; r++) {
            for (int c = 0; c < Nb; c++) {
                state[r][c] = (byte) sbox[state[r][c] & 0xFF];
            }
        }
    }

    // === Step 4: ShiftRows ===
    private void shiftRows() {
        byte[] temp = new byte[4];
        for (int r = 1; r < 4; r++) {
            for (int c = 0; c < Nb; c++) {
                temp[c] = state[r][(c + r) % Nb];
            }
            System.arraycopy(temp, 0, state[r], 0, Nb);
        }
    }

    // === Step 5: MixColumns ===
    private void mixColumns() {
        for (int c = 0; c < Nb; c++) {
            byte a0 = state[0][c];
            byte a1 = state[1][c];
            byte a2 = state[2][c];
            byte a3 = state[3][c];

            state[0][c] = (byte) (mul(0x02, a0) ^ mul(0x03, a1) ^ a2 ^ a3);
            state[1][c] = (byte) (a0 ^ mul(0x02, a1) ^ mul(0x03, a2) ^ a3);
            state[2][c] = (byte) (a0 ^ a1 ^ mul(0x02, a2) ^ mul(0x03, a3));
            state[3][c] = (byte) (mul(0x03, a0) ^ a1 ^ a2 ^ mul(0x02, a3));
        }
    }

    private byte mul(int a, byte b) {
        byte result = 0;
        byte temp = b;
        for (int i = 0; i < 8; i++) {
            if ((a & 1) != 0) result ^= temp;
            boolean high = (temp & 0x80) != 0;
            temp <<= 1;
            if (high) temp ^= 0x1b;
            a >>= 1;
        }
        return result;
    }

    // === Helper Functions ===
    private void loadState(byte[] input) {
        for (int i = 0; i < input.length; i++) {
            state[i % 4][i / 4] = input[i];
        }
    }

    private byte[] dumpState() {
        byte[] output = new byte[16];
        for (int i = 0; i < 16; i++) {
            output[i] = state[i % 4][i / 4];
        }
        return output;
    }
    private static final int[] invSbox = {
            
    };
    // Decrypt a 16-byte block
    public byte[] decryptBlock(byte[] input) {
        loadState(input);
        addRoundKey(Nr);

        for (int round = Nr - 1; round > 0; round--) {
            inverseShiftRows();
            inverseSubBytes();
            addRoundKey(round);
            inverseMixColumns();
        }

        inverseShiftRows();
        inverseSubBytes();
        addRoundKey(0);

        return dumpState();
    }

    // === Inverse SubBytes ===
    private void inverseSubBytes() {
        for (int r = 0; r < 4; r++) {
            for (int c = 0; c < Nb; c++) {
                state[r][c] = (byte) invSbox[state[r][c] & 0xFF];
            }
        }
    }

    // === Inverse ShiftRows ===
    private void inverseShiftRows() {
        byte[] temp = new byte[4];
        for (int r = 1; r < 4; r++) {
            for (int c = 0; c < Nb; c++) {
                temp[(c + r) % Nb] = state[r][c];
            }
            System.arraycopy(temp, 0, state[r], 0, Nb);
        }
    }

    // === Inverse MixColumns ===
    private void inverseMixColumns() {
        for (int c = 0; c < Nb; c++) {
            byte a0 = state[0][c];
            byte a1 = state[1][c];
            byte a2 = state[2][c];
            byte a3 = state[3][c];

            state[0][c] = (byte) (mul(0x0e, a0) ^ mul(0x0b, a1) ^ mul(0x0d, a2) ^ mul(0x09, a3));
            state[1][c] = (byte) (mul(0x09, a0) ^ mul(0x0e, a1) ^ mul(0x0b, a2) ^ mul(0x0d, a3));
            state[2][c] = (byte) (mul(0x0d, a0) ^ mul(0x09, a1) ^ mul(0x0e, a2) ^ mul(0x0b, a3));
            state[3][c] = (byte) (mul(0x0b, a0) ^ mul(0x0d, a1) ^ mul(0x09, a2) ^ mul(0x0e, a3));
        }
    }

}

